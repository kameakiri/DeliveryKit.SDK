...
配送ロジック、ログ基盤、ログアーカイブ、ログ前処理のサンプルを統合した企業向けの開発キットです。
...
- **DeliveryKit.LogArchiver**
  - 180日経過ログの安全な隔離（アーカイブフォルダへ移動）
  - Windows タスクスケジューラで運用可能
  - **移動するだけで、削除はしません。** プライバシーポリシー等で
    「ログは◯日で削除する」と公表する場合、隔離だけでは公表内容を満たしません。
    アーカイブ側の削除も別途組み込んでください（本家DeliveryKitでは、この漏れを
    監査で見つけて、移動と削除の両方を行う形に直しました）
  - 1ファイルの移動失敗（書き込み中でロックされている等）で全体を止めません。
    失敗したものは次回の実行で再試行されます

- **DeliveryKit.AI.Pipeline**
  - ログの正規化
  - ログ行の最低限のクリーンアップ（空行の除去と前後の空白の Trim のみ）。
    **名前の割にしていることは少なく、マスキングも行いません。**
    外部のAIサービスへ渡す前に読むこと → `docs/ai.md`
